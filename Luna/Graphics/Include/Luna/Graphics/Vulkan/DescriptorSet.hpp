...
class DescriptorSetAllocator : public HashedObject<DescriptorSetAllocator>, Utility::NonCopyable {
	constexpr static const int DescriptorSetRingSize = 8;

 public:
	DescriptorSetAllocator(Utility::Hash hash,
	                       Device& device,
	...
	void BeginFrame();
	void Clear();
	std::pair<vk::DescriptorSet, bool> Find(uint32_t threadIndex, Utility::Hash hash);

 private:
	struct DescriptorSetNode : Utility::TemporaryHashMapEnabled<DescriptorSetNode>,
														 IntrusiveListEnabled<DescriptorSetNode> {
	...
	struct PerThread {
		...
		Utility::TemporaryHashMap<DescriptorSetNode, DescriptorSetRingSize, true> SetNodes;
