...
Utility::Hash HashRenderPassInfo(const RenderPassInfo& info, bool compatible = false);

class RenderPass : public HashedObject<RenderPass>, Utility::NonCopyable {
 public:
	RenderPass(Utility::Hash hash, Device& device, const RenderPassInfo& info);
	~RenderPass() noexcept;
...
class Framebuffer : public Cookie, public InternalSyncEnabled, Utility::NonCopyable {
 public:
...
class FramebufferAllocator {
 ...
 private:
	struct FramebufferNode : Utility::TemporaryHashMapEnabled<FramebufferNode>,
													 Utility::IntrusiveListEnabled<FramebufferNode>,
													 public Framebuffer {
		FramebufferNode(Device& device, const RenderPass& renderPass, const RenderPassInfo& renderPassInfo);
	};

	Device& _device;
	Utility::TemporaryHashMap<FramebufferNode, FramebufferRingSize, false> _framebuffers;
...
class TransientAttachmentAllocator {
 ...
 private:
	struct TransientNode : Utility::TemporaryHashMapEnabled<TransientNode>, Utility::IntrusiveListEnabled<TransientNode> {
	...
	Device& _device;
	Utility::TemporaryHashMap<TransientNode, TransientAttachmentRingSize, false> _attachments;
#ifdef LUNA_VULKAN_MT
	std::mutex _mutex;
#endif
};
