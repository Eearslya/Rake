...
namespace Luna {
namespace Vulkan {
Utility::Hash PipelineCompileInfo::GetHash() const {
	Utility::Hasher h;

	const auto& layout  = Program->GetPipelineLayout()->GetResourceLayout();
	ActiveVertexBuffers = 0;
	Utility::ForEachBit(layout.AttributeMask, [&](uint32_t bit) {
	...
	Utility::ForEachBit(ActiveVertexBuffers, [&](uint32_t bit) {
...
void CommandBuffer::BeginRenderPass(const RenderPassInfo& info) {
	...
	_actualRenderPass                         = &_device.RequestRenderPass(Utility::Badge<CommandBuffer>{}, info, false);
	if (info.Name.length() > 0) {
		_device.SetObjectName(_framebuffer->GetFramebuffer(), fmt::format("{} Framebuffer", info.Name));
		_device.SetObjectName(_pipelineCompileInfo.CompatibleRenderPass->GetRenderPass(),
		                      fmt::format("{} (Compatibility)", info.Name));
		_device.SetObjectName(_actualRenderPass->GetRenderPass(), info.Name);
	}
	...
	if (info.Name.length() > 0 && _device.GetExtensionInfo().DebugUtils) {
		const vk::DebugUtilsLabelEXT label(info.Name.c_str(), {0.5f, 0.5f, 1.0f, 1.0f});
...
void CommandBuffer::EndRenderPass() {
	...
	if (_device.GetExtensionInfo().DebugUtils) { _commandBuffer.endDebugUtilsLabelEXT(); }
...
vk::Pipeline CommandBuffer::BuildGraphicsPipeline(bool synchronous) {
	...
	Utility::ForEachBit(attributeMask, [&](uint32_t bit) {
	...
	Utility::ForEachBit(bindingMask, [&](uint32_t bit) {
	...
	Log::Trace("Vulkan::CommandBuffer", "Graphics Pipeline created.");
...
bool CommandBuffer::FlushRenderState(bool synchronous) {
	...
	// Flush descriptor sets.
	{
		...
		Utility::ForEachBit(setUpdate, [&](uint32_t bit) {
			const auto& setLayout = layout.SetLayouts[bit];
			Utility::Hasher h;
			h(setLayout.FloatMask);

			Utility::ForEachBit(setLayout.InputAttachmentMask, [&](uint32_t binding) {
			...
			Utility::ForEachBit(setLayout.UniformBufferMask, [&](uint32_t binding) {
				const auto arraySize = setLayout.ArraySizes[binding];
				for (uint32_t i = 0; i < arraySize; ++i) {
					h(_descriptorBinding.Sets[bit].Cookies[binding + i]);
			...
			Utility::ForEachBit(setLayout.SampledImageMask, [&](uint32_t binding) {
				const auto arraySize = setLayout.ArraySizes[binding];
				for (uint32_t i = 0; i < arraySize; ++i) {
					h(_descriptorBinding.Sets[bit].Cookies[binding + i]);
...

	{
		...
		Utility::ForEachBitRange(updateVBOs, [&](uint32_t binding, uint32_t bindingCount) {
