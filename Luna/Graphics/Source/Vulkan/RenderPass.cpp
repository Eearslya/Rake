...
namespace Luna {
namespace Vulkan {
Utility::Hash HashRenderPassInfo(const RenderPassInfo& info, bool compatible) {
	Utility::Hasher h;
...
RenderPass::RenderPass(Utility::Hash hash, Device& device, const RenderPassInfo& info)
	...
	// Determine our color attachment properties.
	{
		...
		for (uint32_t i = 0; i < _renderPassInfo.ColorAttachmentCount; ++i) {
			...
			if (imageInfo.Domain == ImageDomain::Transient) {
				...
				attachment.initialLayout = image.GetLayout(vk::ImageLayout::eUndefined);
	...
	Utility::StackAllocator<vk::AttachmentReference, 1024> referenceAllocator;
	Utility::StackAllocator<uint32_t, 1024> preserveAllocator;
	...
	std::vector<vk::SubpassDependency> subpassDependencies;
	{
		...
		Utility::ForEachBit(colorSelfDependencies | depthSelfDependencies, [&](uint32_t subpass) {
			const auto subpassBit = 1u << subpass;
			subpassDependencies.emplace_back();
			auto& dep      = subpassDependencies.back();
	...
	Log::Trace("Vulkan::RenderPass", "Render Pass created.");
}
...
Framebuffer::Framebuffer(Device& device, const RenderPass& renderPass, const RenderPassInfo& renderPassInfo)
	...
	Log::Trace("Vulkan::Framebuffer", "Framebuffer created.");
}
...
Framebuffer& FramebufferAllocator::RequestFramebuffer(const RenderPassInfo& info) {
	auto& renderPass = _device.RequestRenderPass(Utility::Badge<FramebufferAllocator>{}, info, true);
