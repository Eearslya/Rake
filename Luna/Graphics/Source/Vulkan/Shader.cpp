...
static std::string MaskToBindings(uint32_t mask, const uint8_t* arraySizes = nullptr) {
	...
	Utility::ForEachBit(mask, [&](uint32_t bit) {
...
PipelineLayout::PipelineLayout(Utility::Hash hash, Device& device, const ProgramResourceLayout& resourceLayout)
...
Shader::Shader(Utility::Hash hash, Device& device, size_t codeSize, const void* code)
...
Program::Program(Utility::Hash hash, Device& device, Shader* vertex, Shader* fragment, const std::string& name)
		: HashedObject<Program>(hash), _device(device), _name(name) {
...
Program::Program(Utility::Hash hash, Device& device, Shader* compute) : HashedObject<Program>(hash), _device(device) {
...
void Program::Bake() {
	...
	for (int i = 0; i < ShaderStageCount; ++i) {
		...
		for (uint32_t set = 0; set < MaxDescriptorSets; ++set) {
			...
			Utility::ForEachBit(activeBinds, [&](uint32_t bit) {
	...
	if (_name.length() > 0) {
		_device.SetObjectName(_pipelineLayout->GetPipelineLayout(), fmt::format("{} (Pipeline Layout)", _name));
	}
...
vk::Pipeline Program::AddPipeline(Utility::Hash hash, vk::Pipeline pipeline) const {
	_device.SetObjectName(pipeline, _name);
	return _pipelines.EmplaceYield(hash, pipeline)->Value;
}

vk::Pipeline Program::GetPipeline(Utility::Hash hash) const {
